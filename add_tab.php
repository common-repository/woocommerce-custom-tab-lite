 ...
 function pctm_add_tab(){
     ...
     if(isset($_REQUEST['eid']))
        {
         ...
         $prior=$prior-31;				echo "vdfg".$res[0]['option_name'];				if(isset($res[0]['option_name'])){			$custom_tab = $res[0]['option_name'];		}else{			$custom_tab = '';		}
              ...
              <table class="form-table">
            	<tbody>
                    <tr class="form-field form-required">
                		...
                		<td><input type="text" aria-required="true" value="<?php echo $custom_tab;?>" id="tab_name" required name="tab_name"></td>
                	</tr>
                     <br />
                	<tr class="form-field form-required">
                		<th scope="row"><label for="description">Custom Tab Content  <span class="description">(required)</span></label></th>
                        
                         <td ><?php wp_editor(isset($content), 'listingeditor' ,$settings = array('textarea_name' => 'tab_description','textarea_rows'=>5,'editor_css' => '<style>  .form-field textarea{ border-width: 0 } </style>') ); ?></td>
    ...
    <div class="wrap">
        <h2 id="add-new-user"> Add Custom Tab <br /></h2>
           <h3> Create New Custom Tab For All Product's</h3>
        <form  id="addtab" name="addtab" method="post"  >
              <table class='form-table'>
            	<tbody>
                    <tr class="form-field form-required">
                		<th scope="row"><label for="tab_name">Custom Tab Label <span class="description">(required)</span></label></th>
                		<td><input type="text" aria-required="true" value="" id="tab_name" required name="tab_name"></td>
                	</tr>
                     <br />
                	<tr class="form-field form-required">
                		<th scope="row"><label for="description">Custom Tab Content  <span class="description">(required)</span></label></th>
